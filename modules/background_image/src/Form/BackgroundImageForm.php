...
class BackgroundImageForm {
  ...
  public function buildImage(array &$group) {
    ...
    if (empty($preload_background_color)) {
      $preload_background_color = $this->backgroundImage->getSettings()->getOverridden('preload.background_color') ?: $calculated_preload_background_color;
    }

    // Dark.
    $dark = $this->getSubformValue(['image', '0', 'background_image', 'dark']);
    if (!isset($dark) || !isset($preload_background_color)) {
      $dark = $this->backgroundImage->getSettings()->getOverridden('dark');
      if (!isset($dark)) {
